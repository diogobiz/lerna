...
import { normalize } from "@diogobiz/utils/dist/Normalize";
  ...
  ({ data: dataProp = dataHelper, onBack }) => {
    ...
    const renderItem = useCallback(
      (item, index) => (
        <SANCollapsePanel
          key={item.title}
          header={<SANTypography fontWeight="bold">{item.title}</SANTypography>}
          customKey={`${index}`}
        >
          <SANTypography>
            <span
    ...
    return (
      ...
      >
        <SANBox>
          ...
          />
          <SANCollapse my="xxl">{data.plataforma.map(renderItem)}</SANCollapse>
          <SANSessionTitle
            title={t("helpCenter.courses.title")}
            subtitle={t("helpCenter.courses.subtitle")}
          />
          <SANCollapse my="xxl">{data.cursos.map(renderItem)}</SANCollapse>
          <SANSessionTitle
          ...
          />
          <SANCollapse my="xxl">{data.outros.map(renderItem)}</SANCollapse>
        </SANBox>
      </SANPage>
    );
  }
