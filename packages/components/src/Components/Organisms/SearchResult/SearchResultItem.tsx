...
const SANSearchResultItem = (props: ISANSearchResultItemProps) => {
  ...
  const getCouters = (item: IItem) => {
    switch (item.type) {
      ...
      case "lesson":
        return (
          !!item.timeInSeconds && `• ${formatSecondsToMin(item.timeInSeconds)}`
        );
      case "resume":
      case "article":
        return (
          ...
          >
            <SANBox flex="1" width="calc(100% - 20px)">
              <SANBox display="flex" alignItems="center">
                ...
                {item.isPopular && (
                  <Badge bg="primary">{t("searchResult.popular")}</Badge>
                )}
              </SANBox>
              <SANTypography
                fontSize="md"
              ...
              {item.course && (
                <SANTypography ellipsis fontSize="sm" color="grey.5" mb="xs">
                  {item.course.name}
                </SANTypography>
              )}
              <SANBox display="flex" alignItems="center">
                <SANBox as="img" mr={2} width={20} src={getIcon(item.type)} />
                <SANTypography
                  fontSize="sm"
                  fontWeight="bold"
                  color="grey.6"
                ...
                {item.professorName && item.type === "lesson" && (
                  <SANTypography fontSize="sm" color="grey.6" ml="xs">
                    • {item.professorName}
                  </SANTypography>
                )}
              </SANBox>
