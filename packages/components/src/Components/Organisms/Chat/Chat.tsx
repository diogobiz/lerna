...
import { createDebounce } from "@diogobiz/utils/dist/Debounce";
...
const SANChat = forwardRef<ISANChatRef, ISANChatProps>(
  ({ hasMore, loadMore, messages, onSend, blocked, loading }, ref) => {
    ...
    useEffect(() => {
      ...
      return () => {
        if (!!scrollRef && !!scrollRef.current) {
          scrollRef.current.removeEventListener("scroll", handleScrollMessages);
        }
      };
    ...
    useEffect(() => {
      ...
      return () => {
        if (!!inputRef && !!inputRef.current) {
          inputRef.current.removeEventListener("input", debounceCountLetter);
        }
      };
    }, [inputRef.current]);
      ...
      <SANBox display="flex" flexDirection="column" minWidth={232}>
        ...
        >
          ...
          <SANBox position="absolute" bottom="0" right="6px">
            <SANTypography fontWeight="bold" fontSize="xs" color={colorCount}>
              {letterCount}/{maxLetters}
            </SANTypography>
          </SANBox>
        </SANBox>
